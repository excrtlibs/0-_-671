...
-- Add this function to your Library table
function Library:CreateKeySystem(title, description, validKeys, onSuccess, onFailure)
    local ScreenGui = Instance.new("ScreenGui")
    ...
    KeyGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Colors.DarkPrimary),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(40, 0, 80)),
        ColorSequenceKeypoint.new(1, Colors.Background)
    }
    KeyGradient.Rotation = 0
    KeyGradient.Parent = KeyFrame
    
    -- Animate gradient rotation
    spawn(function()
        while KeyFrame.Parent do
            TweenService:Create(KeyGradient, TweenInfo.new(8, Enum.EasingStyle.Linear), {Rotation = 360}):Play()
            wait(8)
            KeyGradient.Rotation = 0
        end
    end)
    ...
    TitleLabel.BackgroundTransparency = 1
    TitleLabel.Text = title or "Key System"
    TitleLabel.TextColor3 = Colors.Text
    ...
    DescLabel.BackgroundTransparency = 1
    DescLabel.Text = description or "Enter your key to access the features"
    DescLabel.TextColor3 = Colors.Text
    DescLabel.TextSize = 14
    DescLabel.Font = Enum.Font.Gotham
    ...
    KeyInput.BackgroundColor3 = Colors.DarkPrimary
    ...
    addStroke(KeyInput, 1, Colors.Primary)
    ...
    GetKeyBtn.Size = UDim2.new(0.32, -5, 1, 0)
    ...
    addStroke(GetKeyBtn, 1, Colors.LightPrimary)
    
    -- Discord Button
    local DiscordBtn = Instance.new("TextButton")
    DiscordBtn.Size = UDim2.new(0.32, -5, 1, 0)
    DiscordBtn.Position = UDim2.new(0.34, 0, 0, 0)
    ...
    addStroke(DiscordBtn, 1, Color3.fromRGB(114, 137, 218))
    
    -- Submit Button
    local SubmitBtn = Instance.new("TextButton")
    SubmitBtn.Size = UDim2.new(0.32, -5, 1, 0)
    ...
    addStroke(SubmitBtn, 1, Color3.fromRGB(120, 255, 150))
    ...
    local function setupButtonHover(button)
        button.MouseEnter:Connect(function()
            TweenService:Create(button, TweenInfo.new(0.2), {
                ...
                Size = UDim2.new(button.Size.X.Scale, -2, 1.1, 0)
            }):Play()
        end)
        ...
        button.MouseLeave:Connect(function()
            TweenService:Create(button, TweenInfo.new(0.2), {
                ...
                Size = UDim2.new(button.Size.X.Scale, -5, 1, 0)
            }):Play()
        end)
    ...
    local function validateKey(key)
        key = string.gsub(key, "%s+", "") -- Remove whitespace
        
        for _, validKey in pairs(validKeys or {}) do
    ...
    GetKeyBtn.MouseButton1Click:Connect(function()
        showStatus("Check Discord for keys!", Colors.LightPrimary, 3)
    ...
    DiscordBtn.MouseButton1Click:Connect(function()
        ...
        showStatus("Discord link copied to clipboard!", Colors.LightPrimary, 3)
    ...
    SubmitBtn.MouseButton1Click:Connect(function()
        ...
        if key == "" then
            showStatus("Please enter a key!", Colors.Danger, 3)
            
            -- Shake animation
            for i = 1, 3 do
                TweenService:Create(KeyInput, TweenInfo.new(0.05), {
        ...
        if validateKey(key) then
            showStatus("✓ Valid key! Loading...", Colors.Success, 2)
            ...
            showStatus("❌ Invalid key! Try again.", Colors.Danger, 3)
            ...
            sound.SoundId = "rbxassetid://10066968815"
            sound.Volume = 0.5
            sound.Parent = KeyInput
            sound:Play()
            
            -- Reset after delay
            delay(1, function()
                TweenService:Create(KeyInput, TweenInfo.new(0.3), {
                    BackgroundColor3 = Colors.DarkPrimary,
                    BackgroundTransparency = 0.3
                }):Play()
    ...
    -- Entrance animation
    KeyFrame.Position = UDim2.new(0.5, -225, -1, 0)
    KeyFrame.BackgroundTransparency = 1
    
    TweenService:Create(KeyFrame, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
...
function Library:CreateExampleKeySystem()
    -- Define valid keys
    local validKeys = {
        "PRIDEFALL123",
        "MM2VIP2024", 
    ...
    -- Create the key system
    local keySystem = self:CreateKeySystem(
        "PRIDEFALL MM2", 
        "Enter your key to access premium features\nJoin our Discord to get your key!",
        validKeys,
        
        -- Success callback
        function(key)
            print("Valid key entered:", key)
            -- Load your main GUI here
            self:Notify("Success", "Key validated! Loading GUI...", 3)
            
            -- Example: Create main UI after successful key validation
            local Tab = self:CreateUI("PRIDEFALL MM2", "Premium Features Unlocked!")
            local MainTab = Tab:AddTab(false, "Main", "rbxassetid://6031075931")
            
            self.addButton(MainTab, "Auto Farm", "Automatically collect coins and items", function()
                print("Auto Farm started!")
            end)
            
            self.addButton(MainTab, "Player ESP", "See all players through walls", function()
                print("ESP enabled!")
            end)
        end,
        ...
        function(key)
            ...
            self:Notify("Error", "Invalid key! Check Discord for valid keys.", 3)
        end
    )
